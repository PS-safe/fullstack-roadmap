...
import { Section, TopicCard, InlineCode, Card, Compare, Steps } from '../components/UI';
    ...
    <div className="space-y-12">
      ...
      <Section id="dry" kicker="9.2" title="DRY · KISS · YAGNI">
        ...
        <Compare
          items={[
            {
              label: 'DRY — knowledge, not characters',
              tone: 'a',
              body: (
                <>
                  Extract when two pieces share a <em>reason to change</em> — one concept, one stakeholder. Two functions that look
                  identical today but answer to different stakeholders (a tax rule and a discount rule that both happen to be{' '}
                  <InlineCode>x * 0.9</InlineCode>) will diverge; sharing them couples unrelated features so one team's change breaks
                  the other's. The smell of false DRY is a shared helper with a <InlineCode>kind</InlineCode> flag and growing branches
                  inside it.
                </>
              ),
            },
            {
              label: 'KISS — clever is debt that compounds',
              tone: 'b',
              body: (
                <>
                  Optimize for the person debugging this at 2am, who is often you. The cost isn't writing the clever version, it's that
                  every future reader pays the re-derivation tax. A dumb explicit <InlineCode>switch</InlineCode> beats a one-line lookup
                  nobody can modify safely.
                </>
              ),
            },
            {
              label: 'YAGNI — build the requirement in front of you',
              tone: 'c',
              body: (
                <>
                  Add the parameter, hook, or layer <em>when</em> the case actually arrives, not when you imagine it. The cost of
                  waiting is one refactor later; the cost of speculating is carrying — and testing, and explaining — flexibility nobody
                  uses, often shaped wrong because you guessed before you knew.
                </>
              ),
            },
          ]}
        />
        <Card>
          <h4 className="mb-2 font-semibold">The tension — and the tiebreaker</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            DRY pushes toward abstraction, YAGNI pushes against it. The tiebreaker is the rule of three — duplication is cheaper than
            the wrong abstraction. Wait for the third instance <em>and</em> a confirmed shared intent before extracting. "Duplication
            is far cheaper than the wrong abstraction" (Sandi Metz) is the load-bearing sentence here.{' '}
      ...
      <Section id="layered" kicker="9.3" title="Layered, Hexagonal, Clean, Onion">
        ...
        <Compare
          items={[
            {
              label: 'Layered, or no architecture',
              tone: 'c',
              body: (
                <>
                  Small app or a CRUD service with a thin rules layer → plain layered, or honestly just a controller and an ORM. The
                  classic layered trap: "each layer depends on the one below" quietly lets the domain import the data layer, so you
                  can't test the domain without a database. If you go layered, still point that one arrow inward.
                </>
              ),
            },
            {
              label: 'Hexagonal / Clean / Onion',
              tone: 'b',
              body: (
                <>
                  Real domain logic worth protecting <em>and</em> a concrete need to swap or fake I/O (test without the DB, support a
                  second persistence target, mock an external API). The payoff is a domain test suite that runs in milliseconds with no
                  infrastructure. The tax is more files and more indirection — it needs team buy-in, and on a 200-line CRUD app it's
                  pure overhead.
                </>
              ),
            },
        ...
        />
        <Card>
          <h4 className="mb-2 font-semibold">Three names, one architecture</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            Hexagonal, Clean, and Onion are the <em>same architecture</em> drawn three ways — ports/adapters, concentric circles,
            rings. Pick the vocabulary your team already speaks; don't adopt Clean because Netflix did. The honest cost question: are
            you writing more interfaces than you have implementations? Then you've bought flexibility nobody's using yet.
          </p>
        </Card>
        <MermaidDiagram
          chart={`flowchart TB
      ...
      <Section id="mvc" kicker="9.4" title="MVC, MVP, MVVM">
        ...
        <Compare
          items={[
            {
              label: 'MVC — server-rendered, no binding',
              tone: 'a',
              body: (
                <>
                  Request comes in, controller mutates the model, picks a view, view renders once. Rails, Django, Laravel. The model
                  and view can drift because the round-trip resets everything.
                </>
              ),
            },
            {
              label: 'MVP — view fully unit-testable, no UI framework',
              tone: 'b',
              body: (
                <>
                  The view is "humble" — it has no logic, the presenter sets every value explicitly. You test the presenter against a
                  fake view interface. The cost is verbosity: every field is a manual <InlineCode>view.setX(...)</InlineCode> call.
                </>
              ),
            },
            {
              label: 'MVVM — framework has declarative data-binding',
              tone: 'c',
              body: (
                <>
                  The view-model exposes observable state, the view binds to it, changes propagate automatically — Vue, Angular, WPF.
                  React isn't classic MVVM (no two-way binding), but a component plus its hooks is the same shape: hooks are the
                  view-model, JSX is the view. Picking MVP or MVVM by taste rather than by "do I have binding" is how you end up
      ...
      <Section id="ddd" kicker="9.5" title="Domain-Driven Design">
        ...
        />
        <Card>
          <h4 className="mb-2 font-semibold">Bounded context — the unit that actually matters</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            One model, one team, one ubiquitous language. <InlineCode>Customer</InlineCode> in Sales (cart, payment method, lifetime
            value) and <InlineCode>Customer</InlineCode> in Shipping (name, address, delivery instructions) are{' '}
            <em>different objects that share an id</em> — and that is correct, not a bug to unify. Forcing one shared{' '}
            <InlineCode>Customer</InlineCode> class produces a god object every team has to touch and nobody fully understands.
          </p>
        </Card>
        <Card>
          <h4 className="mb-2 font-semibold">Ubiquitous language is enforced, not aspirational</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            The words in the code are the words the domain expert uses — if they say "policy" and the code says "record", every
            conversation needs a translation step and bugs hide in the gap. The payoff: a domain expert can read a method name and tell
            you it's wrong.
          </p>
        </Card>
        <Steps
          steps={[
            { label: 'one root: Order' },
            { label: 'pull in every LineItem + Customer + Inventory' },
            { label: 'concurrent updates contend on one lock' },
            { label: 'each load drags half the database', tone: 'fail' },
          ]}
          caption={
            <>
              <strong>Aggregate — a consistency boundary, keep it small.</strong> A cluster of entities with one root; the root is the
              only entry point and enforces invariants in <em>one</em> transaction. The failure mode is the greedy aggregate shown
              above. Reference other aggregates by id, not by object.
            </>
          }
        />
        <Card>
          <h4 className="mb-2 font-semibold">Value object — identity-less, immutable, compared by value</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            <InlineCode>Money</InlineCode>, <InlineCode>Address</InlineCode>, <InlineCode>DateRange</InlineCode>. Modelling these as
            plain primitives is where the validation-everywhere smell comes from — make <InlineCode>Money</InlineCode> a type and "can't
            add USD to EUR" becomes a compile error, not a runtime check copy-pasted in twelve places.
          </p>
        </Card>
        <Card>
          <h4 className="mb-2 font-semibold">Domain event — something that happened, past tense</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            <InlineCode>OrderShipped</InlineCode>, <InlineCode>PaymentFailed</InlineCode>. It's how one aggregate tells another the world
            changed without a direct call (this is the seam L7's event-driven and CQRS designs build on).
          </p>
        </Card>
        <Card>
          <h4 className="mb-2 font-semibold">Anti-corruption layer (ACL) — a translator at a context boundary</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            A translator at a context boundary so each side keeps its own language. When you integrate a legacy system or a third-party
            API, the ACL stops their model from leaking into yours — without it, their <InlineCode>Customer</InlineCode> shape slowly
            infects your domain and you've lost the boundary.
          </p>
        </Card>
        <BoundedContextMap />
        <Compare
          items={[
            {
              label: 'No domain worth protecting',
              tone: 'fail',
              body: (
                <>
                  If the app is CRUD with a thin rules layer — a form saves a row, a list reads rows — DDD's aggregates, repositories,
                  and domain events are pure ceremony. You get five files and three indirections for what a controller and an ORM call
                  would do. DDD earns its cost only where the business logic itself is hard: pricing, scheduling, eligibility,
                  settlement.
                </>
              ),
            },
            {
              label: 'Tactical without strategic',
              tone: 'warn',
              body: (
                <>
                  The common failure: a team reads the blue book, adds <InlineCode>Entity</InlineCode> base classes and{' '}
                  <InlineCode>ValueObject</InlineCode> wrappers and a <InlineCode>Repository</InlineCode> per table — but never draws
                  the context boundaries. Result is the same big shared model as before, now with more boilerplate. The boundaries are
                  the value; the building blocks are bookkeeping.
                </>
              ),
            },
          ]}
        />
        <Card>
          <h4 className="mb-2 font-semibold">Rule of thumb</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            Do the strategic work (context map, ubiquitous language) on any non-trivial system — it's just talking to domain experts
            and drawing boxes. Reach for the tactical patterns only inside a context whose logic is genuinely complex.
          </p>
        </Card>
      </Section>
      ...
      <Section id="frontend" kicker="9.6" title="Frontend Architecture & Micro-Frontends">
        ...
        />
        <Card>
          <h4 className="mb-2 font-semibold">Atomic Design — a vocabulary, not a law</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            Atoms → molecules → organisms → templates → pages — a shared vocabulary for a component library, not a law. Its value is
            letting a team say "that's an organism" and agree on scope. The failure mode is treating the taxonomy as the goal:
            agonizing over whether a search field is a molecule or an organism is wasted time. Use it to communicate, don't litigate
            it.
          </p>
        </Card>
        <Card>
          <h4 className="mb-2 font-semibold">Container vs presentational — separate data from looks</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            Separate "where data comes from" from "how it looks". Hooks blurred the mechanical split (you no longer need a wrapper
            component), but the <em>principle</em> holds — a component that both fetches and renders is hard to test and hard to reuse.
            Keep data-access in a hook or a parent, keep the leaf component a pure function of props. This is L4 craft; here it's the
            same boundary rule, scoped to one component.
          </p>
        </Card>
        <Card>
          <h4 className="mb-2 font-semibold">BFF (Backend-for-Frontend) — a per-client server</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            A thin per-client server that aggregates and shapes data — web, iOS, and Android have genuinely different field and
            round-trip needs. The problem it solves is the lowest-common-denominator API: one shared endpoint that over-fetches for
            mobile and under-fetches for web because it serves both. Behind the BFFs the same microservices serve everyone. Cost: one
            more deployable per client to own. Skip it when you have one client, or when GraphQL already lets each client ask for its
            own shape.
          </p>
        </Card>
        <Card>
          <h4 className="mb-2 font-semibold">Micro-frontends — an org-scaling tool</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            Split the app by domain so independent teams ship to one shell (iframes, web components, Module Federation). It's an{' '}
            <em>org-scaling</em> tool — it trades a heavy technical tax (shared-dependency hell, duplicated bundle weight, cross-app
            styling and auth, integration testing across repos) for the ability of many teams to deploy without coordinating.{' '}
            <strong className="text-ink">When NOT to:</strong> one team, or a few. With fewer than ~3 teams that genuinely need
            independent release cadences, you've bought all the tax and none of the benefit — a well-structured modular monolith with
            feature folders gives you the boundaries without the distribution cost. Reaching for micro-frontends without the org
            problem is cargo cult.
          </p>
        </Card>
        <BffDiagram />
        <AtomicDesign />
      ...
      <Section id="patterns" kicker="9.7" title="GoF Design Patterns & Anti-Patterns">
        ...
        <Compare
          items={[
            {
              label: 'The pattern generalizes a case you have once',
              tone: 'warn',
              body: (
                <>
                  Strategy with one strategy, Factory that always returns the same class, Observer with one fixed subscriber — that's
                  an interface and an indirection wrapping a hard-coded call. The pattern earns its keep only at the <em>second</em>{' '}
                  real variant. Until then it's speculative generality: YAGNI at the pattern scale. Test: can you name the second
                  concrete case today? If not, write the direct code and extract the pattern when the case shows up — the refactor is
                  cheap, the wrong abstraction is not.
                </>
              ),
            },
            {
              label: 'The language already has it',
              tone: 'fail',
              body: (
                <>
                  Half the GoF catalog exists to work around 1994 Java. In JS/TS a Strategy is usually just a function passed as an
                  argument; Command is a closure; Iterator is the iterator protocol; Observer is an <InlineCode>EventEmitter</InlineCode>{' '}
                  or a signal. Singleton is almost always a hidden global — a module-scoped value with no DI seam, untestable and
                  order-dependent. Naming the GoF class for something the language gives you for free adds ceremony, not clarity.
                </>
              ),
            },
          ]}
        />
        <Card>
          <h4 className="mb-2 font-semibold">Descriptive, not prescriptive</h4>
          <p className="text-[13px] leading-relaxed text-ink-dim">
            Patterns are descriptive — names for structures that recur — not prescriptive. The skill is recognizing one after the pain
            appears, not installing it in advance. A codebase named after its patterns (
            <InlineCode>AbstractProviderFactoryBean</InlineCode>) is a codebase that confused the vocabulary for the design.
          </p>
        </Card>
        <CodePlayground
          mode="js"
